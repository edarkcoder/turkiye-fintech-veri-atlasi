...
# ─── Yardımcı: JSON benzeri büyük çıktıları temizle ─────────────────────────

def temizle(metin):
    """Ham JSON bloklarını ve çok uzun satırları çıktıdan çıkar."""
    satirlar = []
    for satir in metin.splitlines():
        satir_temiz = satir.strip()
        if satir_temiz.startswith("{") or satir_temiz.startswith("["):
            continue
        if len(satir_temiz) > 300:
            continue
...
def notebook_calistir(dosya_adi):
    ...
    if sonuc.returncode != 0:
        hata = sonuc.stderr[:400] if sonuc.stderr else "Bilinmeyen hata"
        return f"❌ Hata:\n{hata}"
    ...
    for cell in nb.get("cells", []):
        for output in cell.get("outputs", []):
            tur = output.get("output_type", "")
            if tur == "stream":
                text = output.get("text", "")
            elif tur == "execute_result":
...
def html_olustur(bolumler):
    ...
    for baslik, icerik in bolumler:
        renk = "#2d6a4f" if "❌" not in icerik else "#c1121f"
        bolum_html += f"""
        <div style="margin-bottom:28px;">
          <h2 style="color:#1a1a2e;border-bottom:2px solid {renk};
...
def email_gonder(html_icerik):
    ...
    msg.attach(MIMEText(html_icerik, "utf-8"))
